...
const migrate = async () => {
    try {
        console.log('Start migration for Guide 14 Prompt Pack...');

        const guideId = 14;

        // 1. Delete existing guide_prompts for this guide
        console.log('Deleting existing guide_prompts...');
        // guide_prompts links to step_id, not guide_id directly.
        ...
        if (stepError) {
            ...
            if (deleteError) console.error('Error deleting guide_prompts:', deleteError);
            else console.log('Deleted existing guide_prompts for steps:', stepIds);
        } else {
            console.log('No steps found for this guide, skipping prompt deletion.');
        }

        // 2. Insert/Update guide_section
        console.log('Upserting guide_section for Prompt Pack...');

        const promptData = [
            {
                "id": "prompt-1",
                "title": "기본 Text-to-Video 마스터",
                "tags": ["Text-to-Video", "기본", "입문"],
                "relatedStep": [1, 2, 3],
            ...
            },
            {
                ...
                "tags": ["Image-to-Video", "일관성", "Kling AI"],
                "relatedStep": [4],
            ...
            },
            {
                ...
                "tags": ["Cinematic", "Camera", "Lighting"],
                "relatedStep": [6],
            ...
            },
            {
                "id": "prompt-4",
                "title": "제품 쇼케이스 템플릿",
                "tags": ["Commercial", "Product", "Quality"],
                "relatedStep": [4, 6],
            ...
            },
            {
                "id": "prompt-5",
                "title": "스토리텔링 시퀀스 설계자",
                "tags": ["Storytelling", "Shorts", "Reels"],
                "relatedStep": [9],
            ...
            },
            {
                "id": "prompt-6",
                "title": "트러블슈팅 & 고급 테크닉",
                "tags": ["Troubleshooting", "Tips", "Runway"],
                "relatedStep": [3, 5, 10],
        ...
        const { error: upsertError } = await supabase
            ...
            .upsert({
                ...
                data: promptData // Supabase JS handles JSON stringification automatically for JSON columns? Or should I pass object? Usually object.
            }, { onConflict: 'guide_id, section_type, section_order' });

        if (upsertError) {
            console.error('Error upserting guide_sections:', upsertError);
        } else {
            console.log('Upserted guide_sections successfully.');
        }
