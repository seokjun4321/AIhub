-- Move Prompts from Step-by-Step (guide_prompts) to Prompt Pack (guide_sections) for Guide 14

-- 1. Create or Update the Prompt Pack section
INSERT INTO public.guide_sections (guide_id, section_type, section_order, title, data)
VALUES (
  14,
  'prompt_pack',
  1,
  'Professional Prompt Pack',
  '[
  {
    "id": "prompt-1",
...
) ON CONFLICT (guide_id, section_type, section_order) 
DO UPDATE SET 
data = EXCLUDED.data;

-- 2. Clean up guide_prompts for this guide (since they are now moved to Prompt Pack)
DELETE FROM public.guide_prompts WHERE guide_id = 14; 
