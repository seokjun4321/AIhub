-- Emergency Rewrite of approve_guide_submission
-- Fixes multi-line data loss by using robust string_to_array(col, chr(10)) splitting
-- Removes potential specific-character issues with E'\n'
...
BEGIN
    ...
    IF v_metadata->>'aiModelId' IS NOT NULL AND (v_metadata->>'aiModelId')::INTEGER > 0 THEN
        v_model_id := (v_metadata->>'aiModelId')::INTEGER;
    ELSE
        -- 2. Fallback: Find first available model
        SELECT id INTO v_model_id FROM ai_models LIMIT 1;
    END IF;
