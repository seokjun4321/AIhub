-- SQL Migration to update Guide 14 Content with Branch and Copy Blocks (FIXED v2)
-- GUIDE 14: Prompt Engineering for AI Video
-- This script updates the guide text and steps using the new content structure.
-- IMPORTANT: This uses dollar-quoted strings ($$..$$) to preserve actual newlines

-- 1. Update Guide Title and Description
UPDATE guides
SET 
...
UPDATE guide_steps
SET
  title = 'ChatGPT로 첫 영상 프롬프트 설계하기',
  goal = $$ChatGPT를 활용해 AI 영상 생성에 최적화된 구조적 프롬프트를 만든다.$$,
  done_when = $$- ChatGPT가 생성한 프롬프트가 5대 핵심 요소(주요 피사체, 액션, 환경, 카메라, 조명)를 모두 포함
- 한 문장 100자 이내로 간결하게 정리됨$$,
  why_matters = $$AI 영상 도구는 "마법의 버튼"이 아닙니다. **프롬프트가 곧 설계도**입니다. 막연히 "멋진 영상 만들어줘"라고 입력하면 엉뚱한 결과가 나오거나 시간과 크레딧을 낭비하게 됩니다.
...
- ChatGPT가 생성한 프롬프트가 마음에 들면 복사해 메모장에 저장
...
UPDATE guide_steps
SET
  title = '무료 AI 도구 계정 만들고 첫 영상 생성하기',
  goal = $$Kling AI 계정을 만들고 Step 1에서 만든 프롬프트로 첫 영상을 생성한다.$$,
 done_when = $$- Kling AI 계정 생성 완료
- 첫 영상 생성 클릭 후 대기열에 등록됨
- 생성된 영상 1개 이상 다운로드 완료$$,
  why_matters = $$프롬프트만 잘 만들어도 반은 성공이지만, **실제 도구에 입력해야 결과**가 나옵니다. Kling AI는 무료 크레딧을 매일 제공하므로 돈 쓰지 않고 매일 연습할 수 있어 초보자에게 최적입니다.
...
UPDATE guide_steps
SET
  title = '프롬프트 반복 개선으로 원하는 결과 얻기',
  goal = $$첫 결과물을 보고 프롬프트를 개선해 더 나은 영상을 생성한다.$$,
  done_when = $$- 동일 아이디어로 최소 3가지 변형 프롬프트 작성
- 그중 1개 이상 "이거다!" 싶은 만족스러운 결과 획득$$,
  why_matters = $$AI 영상 생성에서 **한 방에 완벽한 결과를 얻는 확률은 10% 이하**입니다. 프로들도 8\~12개 변형을 생성해 그중 1\~2개를 선택합니다.
