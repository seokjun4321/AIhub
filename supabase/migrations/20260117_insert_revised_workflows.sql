-- Insert Revised Workflows (9 items found in user request)
...
- **해결책:** AI 페르소나를 SEO 전문가로 설정하여, 단순 키워드 나열이 아닌 ''사용자 의도(Intent)''와 ''여정 단계(Journey Stage)''를 포함한 전략적 키워드를 도출.',
    ...
    '[
        ...
        {"title": "Output", "description": "변환된 데이터를 화면에 띄우거나 시트에 저장."}
    ]'::jsonb,
    ARRAY['OpenAI API Key (GPT-4 권장)'],
    ARRAY['GPT-3.5보다는 GPT-4를 써야 키워드 분석의 논리력이 좋습니다.', '한 번에 너무 많은 키워드를 요청하면 응답이 잘릴 수 있습니다.', '출력 형식이 깨질 경우 ''JSON Output'' 모드 확인 필요.'],
    NOW(),
    0,
    'n8n',
    '1. 위 GitHub 링크 접속 후 ''Copy raw file'' 버튼 클릭.
2. n8n 캔버스에서 `Ctrl+V` (붙여넣기).
...
- **해결책:** 복잡한 표가 포함된 PDF도 텍스트로 완벽히 변환한 뒤, GPT가 핵심 데이터(공급가액, 세액 등)만 추출하여 시트에 기입.',
    ...
    '[
        ...
        {"title": "HTTP Request (LlamaParse)", "description": "PDF 파일을 LlamaParse API로 전송하여 마크다운 변환."},
        {"title": "OpenAI (Extraction)", "description": "텍스트에서 날짜, 업체명, 총액을 JSON으로 추출."},
        {"title": "Google Sheets", "description": "추출된 JSON 데이터를 시트 행에 추가."}
    ]'::jsonb,
    ARRAY['LlamaCloud API Key (무료 티어 제공)', 'OpenAI API Key', 'Google Sheets OAuth2'],
    ARRAY['보안이 중요한 문서는 로컬 LLM 사용 고려.', 'LlamaParse 무료 사용량 초과 시 비용 발생 가능.', '영수증 형식이 매우 특이한 경우 프롬프트 수정 필요.'],
    NOW(),
    0,
    'n8n',
    '1. GitHub Raw 코드 복사 및 n8n 붙여넣기.
2. LlamaCloud 웹사이트에서 API 키 발급 후 HTTP Request 헤더에 입력.'
),
...
- **해결책:** 클라우드 폴더를 ''업로드 슬롯''으로 활용하여 멀티 채널 배포 자동화.',
    ...
    '[
        ...
        {"title": "TikTok Upload", "description": "틱톡 노드에 연결하여 동일하게 업로드 수행."}
    ]'::jsonb,
...
- **해결책:** 수신함에 들어오는 즉시 AI가 내용을 판단하여 태그(라벨)를 부착.',
    ...
    ARRAY['Gmail에서 라벨을 미리 만들어 두어야 합니다.', '민감한 개인정보는 마스킹 처리 필요.', '뉴스레터 등은 필터로 먼저 거르는 게 좋습니다.'],
    NOW(),
    0,
    'n8n',
    '1. GitHub Raw 코드 복사 및 n8n 붙여넣기.
2. Gmail 노드 인증 후 자신의 라벨 ID 확인 및 매핑.'
),
...
- **해결책:** AI는 ''초안 작성''만 하고, 최종 발송 권한은 사람에게 두어 안정성과 효율성을 모두 확보.',
    ...
    ARRAY['n8n 인스턴스가 공개 인터넷에서 접근 가능해야 합니다(Tunnel/Cloud).', '승인 대기 시간 타임아웃 주의.'],
    NOW(),
    0,
    'n8n',
    '1. GitHub Raw 코드 복사 및 n8n 붙여넣기.
...
-- 10. 주식/코인 차트 분석 AI (TradingView + Vision)
(
...
- **해결책:** 시각 정보를 이해하는 AI에게 객관적인 패턴 분석(헤드앤숄더, 지지선 등)을 맡김.',
    ...
    '[
        ...
        {"title": "OpenAI (Vision)", "description": "이미지 분석 수행."},
        {"title": "Notify", "description": "분석 결과 전송."}
    ]'::jsonb,
