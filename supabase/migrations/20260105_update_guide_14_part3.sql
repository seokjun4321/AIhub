-- Guide 14 Update Part 3: Steps 6-11 & Prompts

INSERT INTO steps (guide_id, step_order, title, goal, done_when, why_matters, tips, checklist, content) VALUES
(14, 6, 'Step 6: 카메라 워크와 조명 키워드로 시네마틱 품질 끌어올리기',
 ...
 '- ❌ 실수: 카메라 용어를 한꺼번에 너무 많이 넣음 (예: "Wide tracking dolly crane shot")\n- ✅ 팁: **1개 앵글 + 1개 움직임**만 사용 (예: "Wide tracking shot")\n- ❌ 실수: 한국어로 카메라 용어 입력 (예: "클로즈업")\n- ✅ 팁: 반드시 **영어 원어** 사용\n- ❌ 실수: 조명 키워드를 빼먹음\n- ✅ 팁: 조명이 분위기의 80%. **항상 조명 키워드 1개 이상** 추가',
...
(14, 7, 'Step 7: 영상 품질 향상 - 업스케일과 편집 마무리',
 ...
 '- ❌ 실수: 업스케일 없이 720p 그대로 업로드\n- ✅ 팁: 유튜브/인스타는 **최소 1080p** 필수. 4K면 알고리즘 우대 받음\n- ❌ 실수: 배경음악에 저작권 음원 사용\n- ✅ 팁: YouTube Audio Library, Epidemic Sound 등 **저작권 프리** 음원만 사용\n- ❌ 실수: 자막을 너무 많이, 길게 넣음\n- ✅ 팁: 자막은 **핵심 1~2문장**만. 너무 많으면 산만함',
...
(14, 8, 'Step 8: 프롬프트 라이브러리 구축과 반복 사용 시스템 만들기',
 ...
 '- ❌ 실수: 프롬프트를 저장하지 않고 매번 새로 작성\n- ✅ 팁: **성공한 건 즉시 저장**. 나중에 찾으려면 기억 안 남\n- ❌ 실수: 프롬프트만 저장하고 메타데이터(도구, 성공률) 누락\n- ✅ 팁: **"어느 도구에서 잘 됐는지"** 메모 필수\n- ❌ 실수: 템플릿을 너무 복잡하게 만듦\n- ✅ 팁: 템플릿은 **3개 이하 변수**만. 단순할수록 재사용 쉬움',
...
(14, 9, 'Step 9: 실전 프로젝트 완주 - 15초 스토리텔링 영상 만들기',
 ...
 '- ❌ 실수: 각 클립의 스타일(조명, 색감)이 제각각\n- ✅ 팁: 모든 프롬프트에 **같은 조명 키워드** 사용 (예: 전부 "golden hour")\n- ❌ 실수: 트랜지션을 너무 많이, 화려하게 사용\n- ✅ 팁: 트랜지션은 **최소화**. 자연스러운 Cut이 가장 프로페셔널\n- ❌ 실수: 음악 볼륨이 너무 크거나 작음\n- ✅ 팁: 음악은 **배경**이 목적. 영상보다 조용하게 (-10dB)',
...
(14, 10, 'Step 10: 배치 생성 전략으로 생산성 10배 끌어올리기',
 ...
 '- ❌ 실수: 10개를 하나씩 생성하고 기다림\n- ✅ 팁: **대기열에 한꺼번에 쌓아두고** 다른 작업 병행\n- ❌ 실수: 모든 결과물을 다운로드\n- ✅ 팁: **미리보기에서 1차 선별** 후 베스트만 다운로드 (저장공간 절약)\n- ❌ 실수: 왜 좋은지/나쁜지 기록 안 함\n- ✅ 팁: 선정 이유를 간단히 메모 (나중에 패턴 파악 가능)',
...
(14, 11, 'Step 11: 최종 산출물 점검 및 배포 전략',
 ...
 '- ❌ 실수: 같은 제목/설명을 모든 플랫폼에 복붙\n- ✅ 팁: 플랫폼마다 **문화와 용어**가 다름. 맞춤 최적화 필수\n- ❌ 실수: 업로드 후 방치\n- ✅ 팁: **첫 1시간이 골든타임**. 댓글 적극 답변 → 알고리즘 부스트\n- ❌ 실수: 실패한 영상을 삭제\n- ✅ 팁: 실패도 데이터. **왜 안 됐는지 분석** 후 다음에 반영',
...
-- Prompts Update
DELETE FROM guide_prompts WHERE guide_id = 14;

INSERT INTO guide_prompts (guide_id, label, text) VALUES
(14, '프롬프트 #1: 기본 Text-to-Video 마스터',
$$[카메라 앵글]: A [피사체 + 구체적 묘사], [액션/움직임], in [환경 + 디테일]. [조명 스타일], [분위기 키워드].
...
(14, '프롬프트 #2: Image-to-Video 정밀 제어',
$$[피사체] + [단순한 움직임 동사], [배경 변화 설명].
...
(14, '프롬프트 #3: 시네마틱 카메라 워크 전문가',
$$[전문 카메라 용어] + [전문 조명 용어]: [피사체], [액션], [환경]. [분위기].
...
(14, '프롬프트 #4: 제품 쇼케이스 템플릿',
$$Close-up rotating shot: A [제품명] placed on [표면 재질], [조명 스타일] lighting, camera slowly orbits around the product to reveal [강조할 디테일].
...
(14, '프롬프트 #5: 스토리텔링 시퀀스 설계자',
$$Scene 1 (훅): [카메라]: [피사체], [강렬한 액션], [환경]. [조명].
