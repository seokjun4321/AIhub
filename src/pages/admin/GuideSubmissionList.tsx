...
const GuideSubmissionList = () => {
    const navigate = useNavigate();

    const { data: submissions, isLoading } = useQuery({
        queryKey: ['admin-guide-submissions'],
        queryFn: async () => {
                            ...
                            <div className="divide-y relative">
                                {submissions.map((item: any) => (
                                    <div
                                        ...
                                        onClick={() => navigate(`/admin/guide-submissions/${item.id}`)}
                                    >
                                        <div className="flex items-center gap-4">
                                            <div className="w-2 h-2 rounded-full bg-purple-500" />
                                            <div>
                                                <div className="flex items-center gap-2 mb-1">
                                                    <span className="font-medium text-slate-900">{item.title || "제목 없음"}</span>
                                                    <Badge variant="outline" className="text-xs capitalize">
                                                        {item.submission_type || "new"}
                                                    </Badge>
...
export default GuideSubmissionList;
